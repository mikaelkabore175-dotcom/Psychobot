...
// --- Cache pour Lazy Loading des groupes ---
...
async function getGroupMetadataSafe(sock, jid) {
    ...
    try {
        ...
        console.log(`⚡ [LazyLoad] Groupe chargé : ${metadata.subject} (${jid})`);
        return metadata;
    } catch (err) {
        console.error(`[LazyLoad] Erreur pour ${jid}:`, err.message);
        return null;
    }
}
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {  
        ...
        try {  
            if (command.adminOnly && isGroup) {  
                const groupMetadata = await getGroupMetadataSafe(sock, remoteJid);
                if (!groupMetadata) {
                    return replyWithTag(sock, remoteJid, msg, "⚠️ Impossible de récupérer les infos du groupe pour l'instant.");
                }
